    ...
    function Sys$_ScriptLoader$_loadScriptsInternal() {
        ...
        if (session.scriptsToLoad && session.scriptsToLoad.length > 0) {
            ...
            if (typeof(nextScript.src) === "string") {
                ...
                var headRemObjects.Elements = document.getRemObjects.ElementsByTagName('head');
                if (headRemObjects.Elements.length === 0) {
                     throw new Error.invalidOperation(Sys.Res.scriptLoadFailedNoHead);
                }
                else {
                     headRemObjects.Elements[0].appendChild(scriptElement);
                }
...
Sys._ScriptLoader.readLoadedScripts = function Sys$_ScriptLoader$readLoadedScripts() {
    if(!Sys._ScriptLoader._referencedScripts) {
        ...
        var existingScripts = document.getRemObjects.ElementsByTagName('script');
...
Sys._ScriptLoaderTask = function Sys$_ScriptLoaderTask(scriptElement, completedCallback) {
    ...
    function Sys$_ScriptLoaderTask$execute() {
        ...
        if (headRemObjects.Elements.length === 0) {
             ...
             headRemObjects.Elements[0].appendChild(this._scriptElement);
        }
    }
...
Sys.Net.WebRequest._resolveUrl = function Sys$Net$WebRequest$_resolveUrl(url, baseUrl) {
    ...
    if (!baseUrl || baseUrl.length === 0) {
        var baseElement = document.getRemObjects.ElementsByTagName('base')[0];
